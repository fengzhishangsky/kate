...
class Ebay extends Admin {
	...
	public function index() {
		$res = Db::table('sent_picture')->select();
		//print_r($res);die;
	...
	public function getList(){
		//$model = M("pictrue");
		//Db::query('select * from sent_picture where id=?',[8]);
		$res = Db::table('sent_picture')->select();
		//print_r($res);die;
		$this->assign("data",$res);
	...
	public function add() {
		if (IS_POST) {
			...
			/*Array ( [id] => 26 [name] => photo_list [title] => 图片列表 [length] => 50 [type] => images [value] => [remark] => [is_show] => 1 [extra] => [model_id] => 3 [is_must] => 0 [status] => 1 [update_time] => 1454052339 [create_time] => 1454052339 [validate_rule] => [validate_time] => 0 [error_info] => [validate_type] => 0 [auto_rule] => [auto_time] => 0 [auto_type] => 0 [help] => [option] => )*/
			$data[] = [
				"id" => 50,
				'name' => "photo_list",
				'title' => "图片列表",
	...
	public function remove(){
		$name = $_POST['name'];
		$id = $_POST['id'];
		$path = ROOT_PATH."uploads/kate/";
		
		@unlink($path.$name);
		$res = Db::table("sent_picture")->delete($id);
		return $res;
